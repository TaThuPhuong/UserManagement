...
@AndroidEntryPoint
class RegisterFragment : BaseFragment<ActivityRegisterBinding>(ActivityRegisterBinding::inflate),
                ...
                if (state != null) {
                    when (state) {
                        ...
                        is Resource.Success -> {
                            ...
                            navController.popBackStack(R.id.nav_list, false)
                        }
    ...
    override fun onClick(view: View?) {
        when (view) {
            binding.btnRegister -> {
                if (validateRegister()) {
                    DialogConfirm(mContext, getString(R.string.register_confirm)) { result ->
                        when (result) {
                            1 -> {
    ...
    private fun setupGenderSpinner(genders: List<Gender>) {
        ...
        val genderAdapter = SpinnerCustom(mContext, genderNames)
...
//                    binding.tvErrGender.gone()
    ...
    private fun validateRegister(): Boolean {
        ...
        isValid =
            setErrorIfLengthExceeded(edUserId, maxLengthUserId, userIdError, mContext) && isValid
        isValid = setErrorIfLengthExceeded(edPass, maxLengthPass, passError, mContext) && isValid
        isValid = setErrorIfLengthExceeded(
            edFamilyName, maxLengthFamilyName, familyNameError, mContext
        ) && isValid
        isValid = setErrorIfLengthExceeded(
            edFirstName, maxLengthFirstName, firstNameError, mContext
        ) && isValid
